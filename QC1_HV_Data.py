...
def xml_from_excel4(excel_file):
	...
	Comment=sys.argv[11]
	Run=sys.argv[3]
	root = generateXMLHeader("QC1_HVDRIFT_CONFIG","GEM Drift PCB QC1 HV Drift Configuration",str(location) + " GEM QC1 Drfit Config HV Test",Run,Start,Stop,comment,location,user)
	...
	root = generateXMLHeader("QC1_HVDRIFT_DATA","GEM Drift PCB QC1 HV Drift Data",str(location) + " GEM QC1 Drift Data HV Test",Run,Start,Stop,comment,location,user)
	dataSet = generateDataSet(root,Comment,"1","GEM Chamber",chamber)
	...
	Elog=sys.argv[9]
	#Comment=sys.argv[11]
	root = generateXMLHeader("QC1_HVDRIFT_SUMRY","GEM Drift PCB QC1 HV Drift Summary",str(location) + " GEM QC1 HV Test Summary",Run,Start,Stop,comment,location,user)
	dataSet = generateDataSet(root,Comment,"1","GEM Chamber",chamber)
