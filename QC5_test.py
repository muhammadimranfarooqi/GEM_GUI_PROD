...
Run=cur.execute("select nvl(max(run.RUN_NUMBER), 0) + 1 from cms_gem_core_cond.cond_data_sets dat join cms_gem_core_cond.kinds_of_conditions koc on dat.KIND_OF_CONDITION_ID = koc.KIND_OF_CONDITION_ID join CMS_GEM_CORE_CONSTRUCT.PARTS par on par.PART_ID = dat.PART_ID join CMS_GEM_CORE_COND.COND_RUNS run on dat.COND_RUN_ID = run.COND_RUN_ID where koc.IS_RECORD_DELETED = 'F' and par.IS_RECORD_DELETED = 'F' and (koc.name = 'GEM Chamber QC5 Effective Gain Data' or koc.name = 'GEM Chamber QC5 Effective Gain Config' ) and par.SERIAL_NUMBER = '"+chamber+"'")
Run = [ t for t, in Run ]
print Run[0]
